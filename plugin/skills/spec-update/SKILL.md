...
Detect and sync source document changes into the SpecTeam workflow state.
...
```json
{
  ...
  "files": {
    "./design/spec.md": {
      ...
      "synced_to": ".spec/design/alice/spec.md",
      "synced_at": "<ISO timestamp>"
    }
  }
}
...
3. Read `.spec/COLLABORATORS.md` to get `{me}`'s source directories.

### Step 2 — Detect source changes

1. Read `.spec/last-sync.json` if it exists. If not, treat all source files as new (first update).
...
3. Compare with hashes stored in `last-sync.json`:
   ...
   - **Deleted file** (in last-sync.json but no longer exists) → needs removal from `.spec/design/{me}/`
...
5. If no changes → output `"✅ No source document changes. SpecTeam workflow state is up to date."` and stop.
...
2. For each **modified or deleted** file, check if it is referenced in any divergence entry (match against `design/{me}/{filename}`):
   ...
   **`open` divergence impact**:
   ...
   - ./design/spec.md → references D-001: API style choice (open, blocking)
     File content will change after sync. Recommend re-running /spec-review after sync.
   ```
...
4. If there are `proposed` divergence impacts → **stop and wait for confirmation**:
   ...
   - User cancels → stop, suggest `/spec-align D-{N}`.
...
1. Copy source file to `.spec/design/{me}/{relative_path}` (preserve relative paths, max 2 levels).
2. If the file didn't previously exist in `.spec/design/{me}/`, prepend `<!-- Spec Normalized Document -->` header.
3. If the file existed (modified), update content while preserving the `<!-- Spec Normalized Document -->` header.

**Deleted files**:
1. Remove `.spec/design/{me}/{filename}` from the workspace.
...
1. Read the current `.spec/design/{me}/{file}` content (just synced).
2. Read `.spec/decisions/D-{N}.md` — locate the **[{me}] Change Instructions** block and extract the **Acceptance criterion** field (primary criterion) and **Required changes** (detail).
...
Write updated `.spec/last-sync.json` with:
- Current timestamp
...
1. Run `git add .spec/design/{me}/ .spec/last-sync.json .spec/DIVERGENCES.md` (if modified).
2. Commit: `"[SpecTeam] update — {me} source doc sync: +{added} ~{modified} -{deleted}"`
3. **Automatically trigger `/spec-parse`** to update INDEX.md.
...
- If files affected open divergences → recommend `/spec-review`
- If files affected proposed divergences where `{me}` is proposer → recommend `/spec-align D-{N}` to withdraw and re-propose
- Otherwise → recommend `/spec-push` to share updates
