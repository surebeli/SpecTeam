---
name: spec-sos
short-description: "Emergency Git conflict resolution for SpecTeam"
description: "Resolves underlying Git merge conflicts in the .spec/ directory automatically. Designed as a fallback when pull or push operations fail due to a Git tree conflict. The AI will read the conflicted files, parse the standard <<<<<<< HEAD markers, and intelligently merge SpecTeam metadata (like DIVERGENCES.md, JSON caches) without losing data, then commit the resolution."
user-invocable: true
argument-hint: ""
triggers: []
callable-by: []
...
Emergency fallback to resolve raw Git merge conflicts in the `.spec/` directory.
...
4. If there are conflicts outside of the `.spec/` directory, output a warning that SpecTeam only auto-resolves its own metadata files, and the user must resolve external files manually.

### Step 2 — Intelligent Auto-Resolution

For each conflicted file inside `.spec/`:
...
2. **Apply file-specific resolution rules**:
   ...
   - `INDEX.md`: Re-generate the index completely based on the current tree (discard the conflicted block and run a quick parse logic or just accept one side and advise running `spec-parse` later).
...
2. Run `git commit -m "[SpecTeam] sos — emergency auto-resolution of Git merge conflicts"`
...
2. Recommend the user to run `/spec-review` and `/spec-parse` to ensure the state is fully synchronized.
3. Recommend running `/spec-push` to share the resolved state with the team.
