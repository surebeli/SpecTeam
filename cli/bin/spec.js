...
program
  .name('spec')
  .description('CLI for SpecTeam spec review and decision alignment')
  .version('1.0.0');
...
program
  .command('install')
  .description('Installs SpecTeam workflow skills for local SpecTeam-style review and alignment flows')
  ...
  .action((options) => {
    ...
    console.log(chalk.yellow(`\nYou can now run e.g. '/spec-status' directly in Claude Code!`));
  });
...
program
  ...
  .action(() => {
    ...
    if (!fs.existsSync(SPEC_DIR)) {
      console.log(chalk.yellow('⚠️ .spec/ directory not found. Please run /spec-init in your AI assistant.'));
      return;
    }
...
program
  ...
  .action(() => {
    ...
    if (!fs.existsSync(SPEC_DIR)) {
      ...
      console.log(chalk.green('✅ Created .spec/ directory structure.'));
    }
